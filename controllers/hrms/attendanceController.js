...
// IP Configuration - First 16 bits (first two octets)
const ALLOWED_IP_PREFIX = '49.207'; // Your fixed 16-bit prefix
...
const isIPAllowed = (ip) => {
  ...
  return cleanIP.startsWith(ALLOWED_IP_PREFIX);
};

// Middleware to check IP restriction
const checkIPRestriction = (req, res, next) => {
  ...
  if (!isIPAllowed(clientIP)) {
    ...
    return res.status(403).json({
      error: "Access denied",
      message: "Attendance can only be marked from authorized network",
      yourIP: clientIP,
      allowedPrefix: ALLOWED_IP_PREFIX + ".x.x"
    });
  }
  
  console.log(`✅ IP authorized: ${clientIP}`);
  next();
};

const attendanceController = {

  // Mark manual attendance (with IP restriction)
  markAttendance: async (req, res) => {
    // Check IP first
    const clientIP = getClientIP(req);
    if (!isIPAllowed(clientIP)) {
    ...
    try {
      ...
      const employeeName = `${employee.firstName} ${employee.lastName}`.trim();
      console.log(`✅ Attendance marked for ${employeeName} from IP: ${clientIP}`);
  ...
  // Auto attendance (IN/OUT toggle) with IP restriction
  autoAttendance: async (req, res) => {
    // Check IP first
    const clientIP = getClientIP(req);
    if (!isIPAllowed(clientIP)) {
    ...
    try {
      ...
      if (!existingEmployee) {
        ...
        console.log(`🟢 IN TIME marked for ${employeeName} at ${currentTime.toLocaleTimeString()} from IP: ${clientIP}`);
        ...
        console.log(`🔴 OUT TIME marked for ${employeeName} at ${currentTime.toLocaleTimeString()} from IP: ${clientIP}`);
        console.log(`⏰ Total working hours: ${hours}h ${minutes}m`);
  ...
  // Get attendance records with filters (no IP restriction)
  getAttendanceRecords: async (req, res) => {
    try {
      const { date, employeeId } = req.query;
      let query = {};
...
// Export both the controller and middleware
module.exports = attendanceController;
module.exports.checkIPRestriction = checkIPRestriction;